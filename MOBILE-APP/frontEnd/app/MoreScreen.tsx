...
const MoreScreen: React.FC = () => {
  ...
  const myActivityItems: RowItem[] = [
    { icon: 'bookmark-outline', iconBg: '#EFF6FF', iconColor: '#1E3A5F', label: 'Saved Jobs', badge: 2, onPress: () => router.replace({ pathname: '/JobsScreen', params: userData }) },
    { icon: 'document-text-outline', iconBg: '#EFF6FF', iconColor: '#1E3A5F', label: 'My Documents', onPress: () => {} },
  ];

  const supportItems: RowItem[] = [
    { icon: 'help-circle-outline', iconBg: '#F0FDF4', iconColor: '#16A34A', label: 'Help Center', onPress: () => {} },
    { icon: 'chatbubble-outline', iconBg: '#F0FDF4', iconColor: '#16A34A', label: 'Contact Support', onPress: () => {} },
    { icon: 'information-circle-outline', iconBg: '#F0FDF4', iconColor: '#16A34A', label: 'About', onPress: () => {} },
  ];

  const settingsItems: RowItem[] = [
    { icon: 'settings-outline', iconBg: '#F5F3FF', iconColor: '#2A4A7A', label: 'App Settings', onPress: () => {} },
    { icon: 'shield-outline', iconBg: '#F5F3FF', iconColor: '#2A4A7A', label: 'Privacy Policy', onPress: () => {} },
    { icon: 'globe-outline', iconBg: '#F5F3FF', iconColor: '#2A4A7A', label: 'Language', rightText: 'English', onPress: () => {} },
  ];
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>{myActivityItems.map((item, i) => <MenuRow key={item.label} item={item} isLast={i === myActivityItems.length - 1} />)}</View>

        <View style={styles.sectionLabel}>
          <Text style={styles.sectionLabelText}>SUPPORT</Text>
        </View>
        <View style={styles.card}>{supportItems.map((item, i) => <MenuRow key={item.label} item={item} isLast={i === supportItems.length - 1} />)}</View>

        <View style={styles.sectionLabel}>
          <Text style={styles.sectionLabelText}>SETTINGS</Text>
        </View>
        <View style={styles.card}>{settingsItems.map((item, i) => <MenuRow key={item.label} item={item} isLast={i === settingsItems.length - 1} />)}</View>
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 22, paddingTop: 22, paddingBottom: 24 },
  ...
  card: { backgroundColor: '#fff', marginHorizontal: 16, borderRadius: 16, paddingHorizontal: 16, elevation: 2, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 2 } },
  ...
  badge: { backgroundColor: '#DBEAFE', borderRadius: 12, paddingHorizontal: 8, paddingVertical: 2 },
  badgeText: { fontSize: 12, fontWeight: '700', color: '#1E3A5F' },

  divider: { height: 1, backgroundColor: '#F3F4F6' },

  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 16, marginTop: 20, paddingVertical: 16, borderRadius: 16, borderWidth: 1.5, borderColor: '#FECACA', backgroundColor: '#fff' },
  logoutText: { fontSize: 15, fontWeight: '700', color: '#DC2626' },

  versionWrap: { alignItems: 'center', marginTop: 20, gap: 2 },
  versionText: { fontSize: 12, color: '#9CA3AF' },

  tabBar: { flexDirection: 'row', backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#E5E7EB', paddingBottom: 8, paddingTop: 10, elevation: 8 },
